...
クリーチャーカタログは `feeds` を宣言せず、シート選択画面には出ません。両APIは形式と共有解決処理を提供します。サーバーは現在、ルールセットの戦闘を処理・保存し、行動ごとにシートを更新できます。ただし対応する戦闘画面はまだなく、プレイヤーは従来の画面を使います。`ruleset.json` と宣言済み `catalogs/<id>.json` の検証後、1.26未満では `combat` と新しい `mechanics` キー、1.27未満では `holds` と `creature` を拒否します。古い厳密なスキーマはファイルを読めません。権限追加や、これらを持たないセットへの変更はありません。
