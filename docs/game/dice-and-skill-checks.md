...
- If the ruleset has a wound track and says its penalty applies to rolls, being hurt makes every check harder. Under a pool ruleset it takes that many dice off the pool, never below the ruleset's own minimum, so a badly hurt character still throws something. Under a ruleset that adds dice up, it is a flat penalty on the roll. Either way the check says how much was applied, so you can see why you rolled fewer dice. See [The ruleset sheet](party-and-npcs.md#the-ruleset-sheet).
