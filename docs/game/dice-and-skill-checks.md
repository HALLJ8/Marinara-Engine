...
- A check can be for a party member. The Game Master adds `who="Name"` to the tag, and the Engine uses that character's sheet. Without `who`, the player is checked. A name with no sheet rolls the dice with no modifier; the Engine never borrows someone else's sheet.
