...
- **`mechanics` is validated and not consumed.** The optional block (kind, range, area, targets, friendly fire, amount, damage type, attack roll, save, cost, per-cost step, concentration, reaction) is a closed strict vocabulary so a typo surfaces now rather than when something finally reads it. In this slice the Engine only validates it and the client shows one compact line. Battles still do not read the sheet at all.
