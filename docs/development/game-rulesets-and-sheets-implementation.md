...
## Proposed: catalogs, so nobody types a spell list by hand

Status: a plan, not built. Raised by the first hands-on use of the 5e sheet: entering spells, attacks and class features row by row is miserable, and a community author will want to ship their system's content the same way.

**The idea.** A ruleset may ship **catalogs**: named collections of ready-made entries that fill the sheet's lists. The sheet editor gains an **Add from catalog** button on every list a catalog feeds. It opens a searchable picker with filters, multi-select, and a mark on entries the sheet already has.

**One entry can fill more than one list.** Picking _Fireball_ adds one row to `spells`. Picking _Second Wind_ adds a row to `features` (name and text) and a row to `counters` (one use, recharges on a short rest), because that is what the feature is on a sheet. An entry is therefore `{ id, label, filter values, rows: [{ list, values }] }`, and `values` are validated against the target list's columns when the ruleset is loaded, so a catalog can never write something the sheet could not hold.

**Copy at pick, like copy at setup.** The picked rows are copied into the sheet with a `catalogRef` beside them. The sheet stays self-contained: it travels on the card, stays readable while dormant, and an updated ruleset never rewrites a character. The reference only lets the picker show "already added" and lets a later **Refresh from ruleset** action offer the newer text.

**Filters are declared, not assumed.** A catalog names the entry properties the picker can filter on (for spells: level, school, class list). A filter may name a sheet field to start from, so a wizard's picker opens on wizard spells. Nothing in the Engine knows the words "spell" or "class".

**Where the data lives.** The SRD spell list alone is larger than the 256 KB cap on `ruleset.json`. So a catalog is its own reserved asset, `catalogs/<id>.json`, declared in the package's asset list and hash-pinned like `ruleset.json`, with its own cap, and fetched by the editor only when a picker opens. `ruleset.json` holds just the catalog's header (id, label, the lists it feeds, its filters). Catalog text never goes to a prompt: the Game Master still sees only the names the sheet summary already lists, so catalogs cost no tokens. For the community lanes a repository carries `rulesets/<id>/catalogs/*.json`; a single imported file may carry small catalogs inline.

**Compatibility.** `ruleset.json` is strict, so a file with a `catalogs` key is refused by an Engine that predates it. The package therefore declares the Capability API that introduces catalogs, and an older Engine refuses the install cleanly instead of loading half a ruleset.

**Catalog entries and combat.** A sheet and its catalogs do not make a battle follow the ruleset. Tactical and Classic battles run on the Engine's own combat model today (`CombatSkill`: a range, an area radius, a slot level, a power multiplier against the attack stat), and nothing in a battle reads the ruleset sheet: not its hit points, not its slots, not its spells. Rules-accurate resolution (a save for half damage, upcasting, action economy, which metamagic may touch which spell) is code, and it belongs to the combat handoff's per-ruleset adapters (`game-combat-rulesets-implementation.md`, its slice 5 for `5e-2014`), not to a data file. What a catalog CAN carry is the facts such an adapter needs, so an entry has an optional typed `mechanics` block from the first cut, with a closed vocabulary agreed with that handoff: range, area shape and size, attack roll or save (ability, half on success), damage dice and type, scaling per slot level, targets, concentration. Entering the SRD once is the point. A smaller step is possible before any adapter exists: a bridge that seeds a combatant from the sheet at the start of a battle (live hit points, slots) and turns catalog spells into Engine skills (range in cells, area, slot cost, element), then writes hit points and slots back afterwards. That would make the sheet matter in a battle while the damage math stays the Engine's, and it must say so.
...
**Slices.** 8a, Engine: the catalog header in the schema, the asset loader and route, the picker, regressions with a second non-5e ruleset. 8b, Agents: `ruleset-5e-2014` 0.2.0 with the SRD catalogs. 8c, Engine: level-scaled maximums, Refresh from ruleset, the cast helper.
