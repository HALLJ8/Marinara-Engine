...
저장소는 에이전트, Game Mode 규칙 집합 또는 둘 다 게시할 수 있으며 적어도 한 종류가 있어야 합니다. 루트의 선택 파일 `agents.json`은 다운로드 가능한 에이전트 정의 형식의 배열입니다. 최소 예시:
...
루트 `rulesets` 폴더에는 최대 32개의 `.json` 파일을 둘 수 있습니다. 각 파일은 집합 하나를 담고 256 KB 이하여야 합니다. 하위 폴더는 무시합니다. 미리 보기는 이름, 버전, 지원 범위를 보여 줍니다. GM 지침은 해당 집합의 게임에서 모델로 전달되므로 신뢰하는 제작자의 콘텐츠만 설치하세요.

ID에는 `alice/my-5e`처럼 소유자가 포함되며 공식 집합이나 다른 제작자의 `v20`을 대체하지 않습니다. 가져온 각 버전은 보존되고 게임은 고정된 정확한 버전을 사용합니다. 같은 버전인데 내용이 다르면 덮어쓰지 않고 버전 번호를 올리도록 요청합니다. 잘못된 파일은 설명과 함께 건너뛰고 나머지는 설치할 수 있습니다.

GitHub 저장소 루트 URL만 지원합니다. 미리 보기 전에 압축 파일 크기와 정의를 검증합니다. 동기화는 저장소가 관리하는 프롬프트, 설정, 도구를 미리 보기 값으로 바꾸며 연결과 이미지는 로컬에 둡니다. 원본에서 삭제된 에이전트는 연결을 해제한 일반 로컬 에이전트로 남습니다. 소스를 제거해도 연결만 해제하며 기존 게임을 위한 규칙 집합은 설치된 채로 유지합니다.
