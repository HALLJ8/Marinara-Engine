...
생물 카탈로그는 `feeds`를 선언하지 않으며 시트 선택기에 나오지 않습니다. 두 버전은 형식과 공유 판정 기능을 제공합니다. 서버는 이제 규칙 집합 전투를 처리하고 저장하며 행동마다 시트를 갱신할 수 있습니다. 해당 전투 화면은 아직 없으므로 플레이어는 기존 전투 화면을 사용합니다. `ruleset.json`과 선언한 `catalogs/<id>.json`을 검증한 뒤, 1.26 미만에서는 `combat`과 새 `mechanics` 키를, 1.27 미만에서는 `holds`와 `creature`를 거부합니다. 이전 엄격한 스키마는 파일을 거부합니다. 새 권한이나 해당 필드가 없는 집합의 변경은 없습니다.
