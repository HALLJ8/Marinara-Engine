...
마법사는 규칙집이 직접 전투를 판정하는 경우, 시트의 체력·에너지·능력으로 Marinara 전투를 진행하는 경우, **Combat Preference**에 따라 Marinara 전투를 진행하는 경우를 구분합니다. 규칙집이 전투를 판정하면 메뉴에 캐릭터의 공격과 능력, 각각의 비용이 표시됩니다. 패널에는 턴 순서, 라운드, 상태와 남은 행동 자원이 표시되고 기록에는 실제 계산이 나옵니다. 각 행동은 즉시 시트에 저장되므로 전투 중 탭을 닫아도 결과가 사라지지 않습니다. 이후 GM은 수치가 이미 확정되었다는 안내를 받습니다. **Combat Preference**는 저장되지만 이러한 전투에서는 사용되지 않습니다.
