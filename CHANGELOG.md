...
- Game Mode rulesets can lend their character sheets to battles with a new optional `battle` block: a fight starts with the sheet's resource pool and spell slots, the abilities picked from the ruleset's catalogs become usable skills, and the health lost or regained along with the slots and resources spent are written back when the fight ends. Hit points are carried as a share of the maximum in both directions, because the numbers in battle are Marinara's: a character at half health on the sheet starts at half the battle's health bar, so a 9-point health pool is never dropped into a fight where one hit does 12. Battles still use Marinara's own combat math, attack rolls, saving throws and concentration are not applied, an abandoned fight writes nothing back, and a ruleset without the block leaves combat unchanged. Ruleset packages that ship the block need Capability API 1.22.
