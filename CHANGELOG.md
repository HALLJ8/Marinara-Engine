...
- A Game Mode ruleset can now ship catalogs: ready-made spells, features or gear that a character sheet's lists can be filled from instead of typing every row by hand. A ruleset author writes them into `ruleset.json`, or, in a rules package, into a separate `catalogs/<id>.json` file for a list too long to fit (Capability API 1.21). One entry can fill more than one list, such as a feature plus the counter that tracks its uses, and everything a catalog offers is checked against the sheet it would be written into. Picked rows are copies you can edit afterwards, and none of this text is ever sent to the model. See [Writing Game Mode Rulesets](docs/extending/writing-rulesets.md).
- Sheet lists a catalog can fill now carry an **Add from catalog** button, in the character and persona editors and in a game's **Edit sheet** alike. The picker searches by name and summary, filters on whatever the ruleset declared (and opens on your own character's value where the ruleset says so), marks what the sheet already holds, shows what an entry does in one line, and says what each list would gain before you add anything. A game picks from the version of the ruleset it plays on.
