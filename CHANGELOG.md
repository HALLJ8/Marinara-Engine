...
- A Game Mode fight can now be fought on a wound track. A ruleset that resolves its own combat can point its health at a track of boxes instead of at a pool of points, and say which kind of harm each of its damage types marks. A blow that lands marks boxes rather than taking points off, one for the whole blow or one for every point of it as the ruleset says its own damage rolls are counted, a character whose track is full is down, and the ruleset's own dying rule reads that exactly as it read somebody reaching zero. Healing clears one mark. Temporary points are refused on such a ruleset, because a track has no buffer for them to sit in, and that is said at import rather than quietly ignored in a fight. Battles that borrow the sheet's numbers carry a track as the share of its length that is still clear, and write back marks of its mildest kind. Rulesets whose health is a pool fight exactly as they did.

- Refresh from ruleset now offers a column a picked row does not carry at all, whatever kind of column it is, so a ruleset that adds a column to one of its lists can reach characters made before it. Each new column is its own line in the review, worded as something the row gained, and it can be declined like any other. A column the row already has keeps the rule it has always had: its text is reviewed, and its numbers and switches are yours, including when they hold 0 or are switched off. Before this the only fix was to delete the row and pick it again (#6400).

- A Game Mode ruleset built on dice pools can now let a player spend a resource to change a roll they are about to make, such as a point of will for an automatic success. The ruleset says which pool it comes out of, what one payment costs, whether it buys successes or extra dice, and how many payments a single check may make, which is what stops a full pool buying a roll that cannot be lost. The Game Master says what the player is spending on the check itself rather than in a separate note, because the dice are thrown before any bookkeeping is applied, and the Engine does both at once: it takes the points, adds what they bought, and rolls. If the resource cannot cover it, nothing is spent and the roll is the one it would have been. The record shows what was really paid and how many successes nobody rolled. The Gravewatch example lets a warden spend Resolve this way. Rulesets that say nothing about spending are unchanged.

- A Game Mode ruleset can now have health that is a track of boxes rather than a number. A track on a character sheet can be given levels, each with its own name and its own penalty, and kinds of harm that a mark can be, such as a bruise and a cut. Marks sit on the track worst first, so a worse one takes the higher box and pushes lighter ones down; the penalty in force is the one on the lowest marked box, never the sum of them; marking a full track makes its lightest mark worse instead of adding one; and anything that still cannot land is counted and kept, so a reload does not forget it. Healing clears the lightest marks first. The sheet draws the track as its boxes, each saying what it is, what it costs and what is on it, and the player can mark and clear them by hand, with the penalty in force said once in words. That penalty now reaches the dice: a ruleset names the track it applies to, and a check takes that many dice off the pool, never below the ruleset's own minimum, or applies it as a flat modifier on a summed roll, and the result says what was applied. The Gravewatch example in the authoring guide gains a short track of its own. Ruleset packages with wound tracks need Capability API 1.30; a fight resolved by a ruleset's own combat block still needs a health pool, and rulesets whose tracks are plain numbers are unchanged.
