...
- Game Mode rulesets can lend their character sheets to battles with a new optional `battle` block: a fight starts with the sheet's hit points, resource pool and spell slots, the abilities picked from the ruleset's catalogs become usable skills, and the damage, healing and slots spent are written back when the fight ends. Battles still use Marinara's own combat math, attack rolls, saving throws and concentration are not applied, an abandoned fight writes nothing back, and a ruleset without the block leaves combat unchanged. Ruleset packages that ship the block need Capability API 1.22.
