...
- A Game Mode ruleset that resolves its own fights can now say what one square of a battlefield is worth in its own distance, and a game on such a ruleset set to Tactical combat is fought on a board. The ruleset's own numbers decide everything on it: how far a turn walks, how far a weapon reaches or is thrown (and a weapon that does both is swung in hand and thrown beyond it), how far an ability carries, whether it lands as a burst, a cone or a line (a creature's breath included, so a shape it prints is the shape it lands in), what a wall blocks, what cover adds to the number an attack is rolled against, and whether walking out of somebody's reach lets them strike at you. A shot taken too far off or with a foe at your elbow is harder when the ruleset says so, and the conditions that talk about distance finally mean something. Opponents nobody plays now move: they close the distance, weigh every square they could fight from, and do not walk through three people's reach for a slightly better target. The 5e example gains distances on its weapons and its creatures, and Ember Roads gains one line saying a square is two paces, which is all a small system needs. The battlefield is not drawn on screen yet, so this release is the rules and the saved battle and the board arrives in the next one; a ruleset that says nothing about distance, and any game set to Classic combat, fights exactly as it did. Ruleset packages that carry any of it need Capability API 1.28, and games with no ruleset are unchanged.
- A Game Mode ruleset that resolves its own fights can now say what one square of a battlefield is worth in its own distance, and a game on such a ruleset set to Tactical combat is fought on a board. The ruleset's own numbers decide everything on it: how far a turn walks, how far a weapon reaches or is thrown, how far an ability carries, whether it lands as a burst, a cone or a line, what a wall blocks, what cover adds to the number an attack is rolled against, and whether walking out of somebody's reach lets them strike at you. A shot taken too far off or with a foe at your elbow is harder when the ruleset says so, and the conditions that talk about distance finally mean something. Opponents nobody plays now move: they close the distance, weigh every square they could fight from, and do not walk through three people's reach for a slightly better target. The 5e example gains distances on its weapons and its creatures, and Ember Roads gains one line saying a square is two paces, which is all a small system needs. A ruleset that says nothing about distance, and any game set to Classic combat, fights exactly as it did. Ruleset packages that carry any of it need Capability API 1.28, and games with no ruleset are unchanged.
