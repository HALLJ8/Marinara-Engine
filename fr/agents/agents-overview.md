...
Un paquet marqué **Rules** fournit un ensemble de règles de Game Mode, comme 5e (SRD 5.1), et non un agent ; il n'entre pas dans les comptes d'agents ci-dessus. Choisis-le sous **Rules** à la création d'une partie ; voir [Choisir les règles](../game/getting-started.md#choosing-rules). Il n'a pas d'interrupteur d'activation par chat. Le paquet 5e est un aperçu, actuellement visible uniquement dans le catalogue d'Engine sur la branche `staging`.
