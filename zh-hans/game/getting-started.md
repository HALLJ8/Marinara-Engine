...
向导会区分三种情况：规则集自行结算战斗、使用角色表的生命值、能量和能力进行 Marinara 战斗，或按 **Combat Preference** 进行 Marinara 战斗。规则集自行结算时，菜单会列出角色的攻击和能力及其消耗。面板显示行动顺序、轮次、状态和剩余行动额度，日志展示实际计算过程。每个行动都会立即保存到角色表，因此中途关闭标签页也不会丢失结果。战斗结束后，GM 会得知数值已结算。**Combat Preference** 仍会保存，但不会用于这类战斗。
